	...
	function load(){
		...
		switch_image.addEventListener('touchstart', function(){
			...
			if(top_field == token_amount_field){
				...
				token_amount_field.top = (OS_ANDROID)? 60: 85;
			...
			if(top_field == token_amount_field){
				...
				if(value != 'na'){
					if(btc_xcp_rate == -1 || xcp_btc_rate == -1 || btc_xcp_limit == -1 || xcp_btc_limit == -1 || btc_xcp_fee == -1 || xcp_btc_fee == -1 || btc_xcp_min == -1 || xcp_btc_min == -1){
						...
						if(buy_token === 'XCP'){
							...
							var total_val = ((parseFloat(send_amount) - parseFloat(btc_xcp_fee))  /  btc_xcp_rate);
							total_val = total_val;
							...
							Titanium.API.log('val ' + send_amount);
							Titanium.API.log('fee ' + xcp_btc_fee);
							var total_val = ((parseFloat(send_amount)  - parseFloat(xcp_btc_fee)) / xcp_btc_rate);
				...
				if(value != 'na'){
					if(btc_xcp_rate == -1 || xcp_btc_rate == -1 || btc_xcp_limit == -1 || xcp_btc_limit == -1 || btc_xcp_fee == -1 || xcp_btc_fee == -1 || btc_xcp_min == -1 || xcp_btc_min == -1){
						...
						if(buy_token === 'BTC'){
							var total_val = ((parseFloat(val) - parseFloat(xcp_btc_fee)) / xcp_btc_rate);
							Titanium.API.log('val ' + val);
							Titanium.API.log('fee ' + xcp_btc_fee);
							if(total_val < 0){
								total_val = 0;
							}
							...
							Titanium.API.log('val ' + val);
							Titanium.API.log('fee ' + btc_xcp_fee);
							var total_val = ((parseFloat(val) - parseFloat(btc_xcp_fee)) /  btc_xcp_rate);
		...
		keypad_view.height = display_height - 165;
		keypad_view.top = 165;
